...
import { triggerManualTick, resetAgentBrain, resetWorldState } from "../lib/server-functions";
...
export default function AdminPanel() {
  ...
  const handleFullReset = async () => {
    if (!confirm("CRITICAL: This will delete ALL agents, memories, events, and relationships. Are you sure?")) return;
    
    setLoading(true);
    setStatus("Wiping world...");
    try {
      await resetWorldState();
      setStatus("World re-seeded!");
    } catch (e) {
      setStatus("Reset failed.");
      console.error(e);
    }
      ...
      <div className="p-3 space-y-4">
        <div className="grid grid-cols-2 gap-2">
          <button
            onClick={handleTick}
            disabled={loading}
            className="flex-1 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-[10px] font-bold rounded-lg flex items-center justify-center gap-2 transition-colors"
          >
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 py-2 bg-red-900/40 hover:bg-red-900/60 border border-red-500/30 disabled:opacity-50 text-red-400 text-[10px] font-bold rounded-lg flex items-center justify-center gap-2 transition-colors"
          >
